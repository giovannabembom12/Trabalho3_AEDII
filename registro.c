...
static const char *NOMES[] = {
    ...
    "Vithor", "Manfred", "Maria", "João", "Iago"
};
static const char *CATEGORIAS[] = {
    "Eletrônicos", "Alimentos", "Vestuario",
    "Moveis", "Livros"
};
#define N_NOMES      10
...
int arquivo_busca_seq_ano(FILE *arq, int limiar, int op)
{
    ...
    while (fread(&reg, TAM_REGISTRO, 1, arq) == 1) {
        int ok = 0;
        switch (op) {
            case 0: ok = (reg.ano >  limiar); break;
            case 1: ok = (reg.ano >= limiar); break;
            case 2: ok = (reg.ano <  limiar); break;
            case 3: ok = (reg.ano <= limiar); break;
        }
        if (ok) count++;
    }
    return count;
